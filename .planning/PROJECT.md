...
- [ ] App validates required credentials at startup and fails with a message naming the missing variable and where to obtain it
...
renovations pay off most as flight and passenger capacity grows.
...
| All credentials are REQUIRED in `.env`; no degraded or fallback path is built | Reversed an earlier "must run keyless" decision. Requiring keys removes the keyword-dispatcher and no-key mode from scope entirely, buying back hours for scoring depth — the thing actually being graded. Accepted cost: a reviewer without keys sees a startup error, not a partial demo. Mitigated by `.env.example`, a clear README, and Gemini's free no-credit-card tier | — Pending |
| Startup fails loudly and specifically when a credential is missing | If there is no fallback path, a vague failure is the worst outcome. The error must name which variable is absent and where to get it | — Pending |
...
| Airport identity/geometry sourced from the FAA ArcGIS API at startup, cached — no bundled CSV | Reconsidered and reversed an earlier "bundle a CSV" decision. We already call FAA ArcGIS for runway data, so identity from the same source means one fewer data source and zero bundled datasets — the truest reading of live-APIs-only. Bootstrapped once into cache, so it is not a per-request call, and the fetched list still provides the SSRF allowlist | — Pending |
...
| LLM: Gemini 2.5 Flash **only**, via Vercel AI SDK — no second provider | Reversed an earlier dual-provider plan. Flash is free with no credit card, so both the user and the reviewer can actually run it; it is more than capable of intent parsing and narrating pre-computed numbers. The dual-provider fallback was self-defeating: its own justification was that the AI SDK makes swapping a one-line change, which is precisely why building both up front is unnecessary. Accepted cost: free-tier rate limits (~10 rpm) and no second path if Gemini is down | — Pending |

...
- **[New, 2026-08-23]** UAT for Phase 4 was independently re-run live against the running dev server (not just user self-report): QUERY-01 (ranked list) confirmed. QUERY-02 (KPI comparison) returned both values plus a ratio ("1.8 times") but never the plain subtractive numeric difference the test wording asked for — accepted as-is per user direction, not fixed. CHAT-03 (follow-up resolution) initially could not be checked in the same session (daily Gemini quota exhausted mid-test), but was independently re-confirmed later the same day on a fresh dev server instance: "what about Boston?" correctly resolved to KBOS (the actual top-ranked airport from turn 1), reused the same data window, and correctly labeled its long-haul-threshold and runway-configuration judgments as its own estimate per SYSTEM_PROMPT. All 3 UAT tests now independently confirmed against the live app, not just self-reported.
...
| 260901-g0s | An empty OpenSky window is a measured zero, not a failure — a zero-traffic airport now keeps all three scoring components and ranks last instead of scoring 50 | 2026-09-01 | uncommitted | [260901-g0s-opensky-empty-result-is-a-measured-zero-](./quick/260901-g0s-opensky-empty-result-is-a-measured-zero-/) |
